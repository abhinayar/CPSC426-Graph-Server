...
int json_emit_unsigned_long(char *buf, int buf_len, uint64_t value) {
  char tmp[20];
    ...
    switch (*fmt) {
      ...
      case ' ': case '\r': case '\n': case '\t':
        ...
        break;
			// Begin Added by Bo
			case 'u':
				s += json_emit_unsigned_long(s, end - s, va_arg(ap, uint64_t));
				break;
			// Added by Bo End
      case 'i':
...
// Begin Added by Bo
int mg_http_dispatch(const char *buf, int len, char *dst, int dst_len,
                    const char **methods, const char **uris, struct mg_str http_uri, mg_rpc_handler_t *handlers) {
  struct json_token tokens[200];
  struct mg_rpc_request req;
  int i, n;
  ...
  if (methods[i] == NULL) {
	...
	for(i = 0; uris[i] != NULL; i++) {
		int ulen = strlen(uris[i]);
		if (ulen == http_uri.len && 
				memcmp(uris[i], http_uri.p, ulen) == 0)
			break;
	}
